...
PacketHandler::PacketHandler(SOCKET S, char *ip)
{
	...
	for (int i = 0; i < 16; i++)LocIP[i] = 0; //locIP에서 바꿈
	SetClientIP(ip);
...
void PacketHandler::GenerateResponse(int ResponsePacketType)
{
	...
	case JOIN_PLAYERDATA_RESPONSE:
		...
		if (MySql.IsNewDayLogin(Info.usr_id)) //코드에서 캐쉬로
		{
	...
	case SHOP_BUY_RESPONSE:
		...
		if (Shop_Buy_Request->slot == -1)
		{
			...
			case 2503:
				...
				MySql.InsertNewItem(&Info, 2016, 6000, 1, 0);
				break;
			case 2504: //유상 대신 스11 21 22 10장씩
				...
				break;
			case 2505: //유상 대신 펫 레오, 드래곤, 비글, 펭귄 등
	...
	case NPC_KILL_RESPONSE:
		...
		QuestGain_Response.eleType = Random::UInt(1, 4);//퀘스트 정령타입
		QuestGain_Response.eleBase = Random::UInt(1, 5);//퀘스트 정령 갯수 배율
		QuestGain_Response.eleMul = Random::UInt(100, 150);//퀘스트 정령X배율
		RoomList.BroadcastExpGain(&QuestGain_Response, Info.usr_room);
		nOfPackets = 0;
		break;
		/* 이부분 활성화 시키면 무한 퀘스트라이프
	...
	case ROOM_EXIT_RESPONSE:
		for (int i = 0; i < MaxRoom; i++)
		{
			if (RoomList.Rooms[i].n == Info.usr_room)
			{
				if (Info.usr_team == 10) RoomList.Rooms[i].blueteam--; //방나올때 팀 초기화
				else if (Info.usr_team == 20) RoomList.Rooms[i].redteam--; //방나올때 팀 초기화
				break;
			}
		}
		GetExitRoomResponse();
	...
	break;
	/*case TRADE_SUCCESS_RESPONSE:  소스제작중 
	{
